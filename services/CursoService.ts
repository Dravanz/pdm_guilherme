...
export class CursoService {
  ...
  static async obterXMLCurso(cursoId: string, userPhotoUrl?: string): Promise<string> {
    try {
      ...
      Object.keys(storageImages).forEach(key => {
        xmlContent = xmlContent.replace(new RegExp(`{{${key}}}`, 'g'), storageImages[key as keyof typeof storageImages]);
      });
  ...
  private static obterXMLCursoFallback(cursoId: string, userPhotoUrl?: string): string {
    return '';
  }
  ...
  static async atualizarCoeficienteTotalUsuario(usuarioId: string): Promise<void> {
    try {
      ...
      const coeficienteGeral = totalQuestoesSistema > 0 
