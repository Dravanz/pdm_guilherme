...
export class RankingService {
  static async obterRanking(): Promise<RankingData> {
    try {
      // Primeiro tenta buscar ranking cached (se usuário está autenticado)
      if (auth.currentUser) {
        const rankingCached = await this.obterRankingCached();
        if (
          rankingCached &&
          this.rankingEstaAtualizado(rankingCached.ultimaAtualizacao)
        ) {
  ...
  static async gerarNovoRanking(): Promise<RankingData> {
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        usuarios.push({
          ...
          coeficienteConhecimento:
            typeof coeficiente === "number" && !isNaN(coeficiente)
              ? coeficiente
              : 0,
          posicao: posicao,
      ...
      // Verificar e conceder badges de ranking (apenas se usuário autenticado)
      if (auth.currentUser) {
  ...
  static async obterRankingCached(): Promise<RankingData | null> {
    try {
      ...
      if (rankingSnap.exists()) {
        ...
        const usuarios = (data.usuarios || []).map(
  ...
  static async salvarRankingCache(ranking: RankingData): Promise<void> {
    try {
      // Só salvar cache se houver usuário autenticado
      if (!auth.currentUser) {
        return;
      }
  ...
  static rankingEstaAtualizado(ultimaAtualizacao: Date): boolean {
    ...
    const diferencaMinutos =
      (agora.getTime() - ultimaAtualizacao.getTime()) / (1000 * 60);
    return diferencaMinutos < 1.5; // Atualiza a cada 1 minuto e meio
  }

  static async verificarBadgesRanking(
    usuarios: RankingUsuario[]
  ): Promise<void> {
    try {
      ...
      for (const usuario of usuarios) {
        ...
        const badgesUsuario = badgesExistentes.filter(
          (b) => b.usuarioId === usuario.uid
        );

        // Badges específicas de posição
        if (
          usuario.posicao === 1 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_1")
        ) {
          await BadgeService.concederBadgeRanking(usuario.uid, "ranking_1");
        }
        if (
          usuario.posicao === 2 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_2")
        ) {
          await BadgeService.concederBadgeRanking(usuario.uid, "ranking_2");
        }
        if (
          usuario.posicao === 3 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_3")
        ) {
        ...
        if (
          usuario.posicao <= 5 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_top5")
        ) {
          await BadgeService.concederBadgeRanking(usuario.uid, "ranking_top5");
        }
        if (
          usuario.posicao <= 10 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_top10")
        ) {
          await BadgeService.concederBadgeRanking(usuario.uid, "ranking_top10");
        }
        if (
          usuario.posicao <= 20 &&
          !badgesUsuario.find((b) => b.badgeId === "ranking_top20")
        ) {
